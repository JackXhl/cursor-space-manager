...
> Status: v0.1.1, **Windows first**. macOS/Linux platform code is in the tree
...
`latest.json`). Installers are named `Cursor_Space_Manager_<version>_x64-setup.exe`
(and a matching MSI). After the build succeeds, the workflow checks that
`latest.json`, `.sig` files, and the Windows updater entry exist — and writes
`latest.json` itself if the uploader skipped it — then **publishes the draft
automatically**. There is no extra click on GitHub.

...
export const useMigrationStore = defineStore('migration', () => {
  ...
  async function execute() {
    ...
    try {
      await refreshQuiescence()
      if (quiescence.value?.quiet !== true) {
        running.value = false
        step.value = 'confirm'
        useAppStore().notify(quiescence.value?.message ?? t('confirm.quitFirst'), 'warning')
        return
      }
  ...
  async function restoreRoot(operationId: string, rootId: string) {
    const app = useAppStore()
    try {
      await api.restoreRoot(operationId, rootId)
      app.notify(t('recovery.restoredToast'), 'success')
      await app.refreshRecovery()
    } catch (error) {
      app.notify(toCommandError(error).message, 'danger')
    }
  ...
  async function discardOrphanedCopy(operationId: string, rootId: string) {
    const app = useAppStore()
    try {
      await api.discardOrphanedCopy(operationId, rootId)
      app.notify(t('recovery.discardedToast'), 'success')
      await app.refreshRecovery()
    } catch (error) {
      app.notify(toCommandError(error).message, 'danger')
    }

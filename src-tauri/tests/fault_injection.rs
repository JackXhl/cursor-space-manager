...
#[test]
fn leftover_target_with_intact_source_can_be_discarded() {
    let fixture = Fixture::new();
    let plan = fixture.plan(&["cache"]);
    let root = &plan.roots[0];
    let expected = snapshot(&root.source_path);
    ...
    std::fs::create_dir_all(&root.target_path).unwrap();
    std::fs::write(root.target_path.join("extra.txt"), b"stale").unwrap();

    fixture
        .executor(quiet())
        .discard_orphaned_copy(&plan, &root.root_id)
        .unwrap();
...
#[test]
fn discard_refuses_when_the_source_is_already_a_link() {
    let fixture = Fixture::new();
    let plan = fixture.plan(&["cache"]);
    let root = &plan.roots[0];
    fixture.executor(quiet()).run(&plan, &mut |_| {}).unwrap();

    let error = fixture
        .executor(quiet())
        .discard_orphaned_copy(&plan, &root.root_id)
        .expect_err("a live migration must not look like leftover staging");
    assert!(error.to_string().contains("不会删除"));
    assert!(root.target_path.exists());
}

#[test]
fn undo_refuses_when_the_link_points_somewhere_else() {
    let fixture = Fixture::new();
    let plan = fixture.plan(&["cache"]);
    let root = &plan.roots[0];
    let expected = snapshot(&root.source_path);
    ...
    let error = executor
        .restore_root(&plan, &root.root_id)
        .expect_err("an unexpected link must not be removed automatically");
    assert!(error.to_string().contains("不会自动改动") || error.to_string().contains("不明确"));
    assert!(root.backup_path.exists());
    assert_eq!(snapshot(&root.backup_path), expected);
}

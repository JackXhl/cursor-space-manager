...
impl Executor {
    ...
    fn require_root<'a>(plan: &'a MigrationPlan, root_id: &str) -> AppResult<&'a PlannedRoot> {
        plan.roots
            .iter()
            .find(|root| root.root_id == root_id)
            .ok_or_else(|| AppError::not_found(format!("计划中没有目录 {root_id}")))
    }

    /// Sources must still be ordinary directories. A path that became a link
    /// while we planned would make robocopy copy the wrong tree.
    fn assert_sources_plain(&self, plan: &MigrationPlan) -> AppResult<()> {
        for root in &plan.roots {
            match platforms::link_state(&root.source_path) {
                LinkState::Regular => {}
                LinkState::Missing => {
    ...
    fn refuse_unexpected_link(&self, root: &PlannedRoot) -> AppResult<()> {
        let state = platforms::link_state(&root.source_path);
        if !state.is_link() {
            return Ok(());
        }
        match state.target() {
            Some(target) if crate::util::paths_equal(target, &root.target_path) => Ok(()),
            Some(target) => Err(AppError::unsafe_op(format!(
                "{} 指向 {}，与这次搬家的目标不一致，不会自动改动",
                root.source_path.display(),
                target.display()
            ))),
            None => Err(AppError::unsafe_op(format!(
                "{} 是无法识别的链接，不会自动改动",
                root.source_path.display()
            ))),
        }
    ...
    pub fn run(&self, plan: &MigrationPlan, progress: ProgressSink) -> AppResult<MigrationResult> {
        ...
        if let Err(error) = self.assert_sources_plain(plan) {
            self.fail_safe(plan, &error)?;
            return Err(error);
        }
    ...
    pub fn rollback_root(&self, plan: &MigrationPlan, root: &PlannedRoot) -> AppResult<()> {
        let operation_id = &plan.operation_id;
        let root_id = root.root_id.as_str();
        self.refuse_unexpected_link(root)?;
        self.journal
    ...
    pub fn undo(&self, plan: &MigrationPlan) -> AppResult<MigrationResult> {
        ...
        for root in &plan.roots {
            ...
            let result = if backup_present {
                ...
                self.refuse_unexpected_link(root)
                    .and_then(|_| self.restore_from_target(plan, root))
            };
    ...
    /// Restores one root according to what is actually on disk.
    ///
    /// Leftover copies with an intact source are not restored here; those must
    /// go through `discard_orphaned_copy` so we never overwrite live data.
    pub fn restore_root(&self, plan: &MigrationPlan, root_id: &str) -> AppResult<()> {
        let root = Self::require_root(plan, root_id)?;
        let diagnosis = crate::core::recovery::diagnose_root(root);
        match diagnosis.disposition {
            crate::core::recovery::RootDisposition::SourceRenamedOnly
            | crate::core::recovery::RootDisposition::MigratedWithBackup
            | crate::core::recovery::RootDisposition::MigratedNoBackup => {}
            crate::core::recovery::RootDisposition::StagedOnly => {
                return Err(AppError::unsafe_op(format!(
                    "{} 的原文件夹还在，请改清理残留副本，不要还原",
                    root.label
                )));
            }
            crate::core::recovery::RootDisposition::Untouched => {
        ...
        let report = crate::core::recovery::diagnose(&self.journal, &plan.operation_id)?;
        let still_open = report.roots.iter().any(|root| {
            matches!(
                root.disposition,
                crate::core::recovery::RootDisposition::MigratedWithBackup
                    | crate::core::recovery::RootDisposition::MigratedNoBackup
                    | crate::core::recovery::RootDisposition::SourceRenamedOnly
                    | crate::core::recovery::RootDisposition::UnexpectedLink
                    | crate::core::recovery::RootDisposition::DataMissing
            )
        });
        if !still_open {
            self.journal
                .set_operation_state(&plan.operation_id, MigrationState::RolledBack)?;
            self.journal
                .set_backups_retained(&plan.operation_id, false)?;
        }
        Ok(())
    ...
    /// Deletes a leftover target copy when the original directory is still intact.
    pub fn discard_orphaned_copy(&self, plan: &MigrationPlan, root_id: &str) -> AppResult<()> {
        let root = Self::require_root(plan, root_id)?;
        let diagnosis = crate::core::recovery::diagnose_root(root);
        if diagnosis.disposition != crate::core::recovery::RootDisposition::StagedOnly
            || !diagnosis.safe_to_automate
        {
            return Err(AppError::unsafe_op(format!(
                "{} 现在不是「原文件夹完好、只有残留副本」的状态，不会删除",
                root.label
            )));
        }

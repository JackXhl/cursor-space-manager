...
/// Rejects a plan whose sources no longer match the latest scan.
///
/// The UI keeps the plan id between Confirm and Execute; a rescan or a
/// competing tool can change those paths in that window.
pub fn assert_plan_current(plan: &MigrationPlan, scan: &ScanReport) -> AppResult<()> {
    if !plan.blockers.is_empty() {
        return Err(AppError::unsafe_op("还有问题需要先解决，暂时不能开始"));
    }

    for planned in &plan.roots {
        let Some(scanned) = scan.roots.iter().find(|root| root.id == planned.root_id) else {
            return Err(AppError::unsafe_op(format!(
                "{} 已不在最新扫描中，请重新扫描后再搬家",
                planned.label
            )));
        };
        if !paths_equal(&scanned.path, &planned.source_path) {
            return Err(AppError::unsafe_op(format!(
                "{} 的路径已变化，请重新扫描后再搬家",
                planned.label
            )));
    ...
    if let Some(volume) = scan
        .volumes
        .iter()
        .find(|volume| volume.mount.eq_ignore_ascii_case(&plan.target_volume))
    {
        if !volume.eligible_target {
            return Err(AppError::unsafe_op(
                volume
                    .ineligible_reason
                    .clone()
                    .unwrap_or_else(|| "目标磁盘不再适合作为迁移目标".into()),
            ));
        }
    }
...
mod tests {
    ...
    #[test]
    fn a_stale_source_path_is_rejected() {
        let temp = tempfile::tempdir().unwrap();
        let source = temp.path().join("source");
        std::fs::create_dir_all(&source).unwrap();
        ...
        plan.blockers.clear();
        report.roots[0].path = moved;
        report.roots[0].resolved_path = report.roots[0].path.clone();
        let error = assert_plan_current(&plan, &report).unwrap_err();
        assert!(error.to_string().contains("路径已变化"));
    }

...
pub fn running_processes() -> Vec<RunningProcess> {
    ...
    for (pid, process) in system.processes() {
        ...
        if !crate::platforms::is_cursor_process(&name, exe_path.as_deref()) {
            continue;
        }
...
pub fn discover_installation() -> AppInstallation {
    ...
    installation.install_dir = Some(dir.clone());
    installation.exe_path = Some(exe);
    installation.conflicts = found
        .iter()
        .skip(1)
        .map(|(_, d, _)| d.clone())
        .filter(|path| !crate::util::paths_equal(path, &dir))
        .collect();
    installation.running = !processes.is_empty();
    installation.processes = processes;
    installation

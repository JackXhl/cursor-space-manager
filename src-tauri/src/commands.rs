...
pub fn check_quiescence(state: State<'_, AppState>) -> QuiescenceReport {
    let mut processes = crate::platforms::running_processes();
    let mut lock_error: Option<String> = None;
    if let Some(plan) = state.active_plan.read().clone() {
        for root in &plan.roots {
            match crate::platforms::processes_locking(&root.source_path) {
                Ok(holders) => {
                    for holder in holders {
                        if !processes.iter().any(|process| process.pid == holder.pid) {
                            processes.push(holder);
                        }
                    }
                }
                Err(error) => {
                    lock_error = Some(error.to_string());
                }
            }
        }
    }

    let quiet = processes.is_empty() && lock_error.is_none();
    QuiescenceReport {
        message: if let Some(error) = lock_error {
            format!("无法确认文件是否仍被占用（{error}），暂不搬家")
        } else if quiet {
            "未检测到运行中的 Cursor 进程".to_string()
        } else {
            format!(
                "仍有 {} 个 Cursor 相关进程在占用文件，请完全退出后继续",
                processes.len()
            )
        },
        quiet,
...
pub fn start_migration(
    ...
    if let Some(scan) = state
        ...
        .read()
        .clone()
        .or_else(|| state.journal.latest_scan().ok().flatten())
    {
        planner::assert_plan_current(&plan, &scan).map_err(AppErrorWire)?;
    }
...
pub fn restore_root(
    ...
    let plan = load_plan(&state, &operation_id).map_err(AppErrorWire)?;
    let executor = Executor::new(Arc::clone(&state.journal), Arc::new(AtomicBool::new(false)));
    executor.restore_root(&plan, &root_id).map_err(AppErrorWire)
}

#[tauri::command]
pub fn discard_orphaned_copy(
    state: State<'_, AppState>,
    operation_id: String,
    root_id: String,
) -> CommandResult<()> {
    ...
    let executor = Executor::new(Arc::clone(&state.journal), Arc::new(AtomicBool::new(false)));
    executor
        .discard_orphaned_copy(&plan, &root_id)
        .map_err(AppErrorWire)
}

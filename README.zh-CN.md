...
> 状态：v0.1.1，**Windows 优先**。macOS/Linux 适配代码已在仓库中，但尚未纳入公开发版矩阵。
...
版本号来自构建元数据，不能通过配置文件篡改。推送 `v*` tag 会跑 `.github/workflows/release.yml`：Windows 先把安装包写进 **草稿** Release（避免更新器读到半成品 `latest.json`）。安装包文件名为 `Cursor_Space_Manager_<version>_x64-setup.exe`（另有对应 MSI）。成功后，工作流会校验 `latest.json`、`.sig` 以及 Windows 更新条目；若上传器漏了 `latest.json` 会补写一份，再 **自动把草稿改成正式版**，不用再去 GitHub 点 Publish。
